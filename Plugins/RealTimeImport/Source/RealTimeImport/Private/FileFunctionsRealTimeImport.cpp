...
void UFileFunctionsRealTimeImport::LoadAllFilesFromDirectory(ERTIDirectoryType directoryType, ERTIFileType fileType, FString dirPath, bool recursiveSearch, int32& count, TArray<FString>& files, TArray<FString>& filePaths) {
...
void UFileFunctionsRealTimeImport::SaveFile(ERTIDirectoryType directoryType, FString filePath, bool createDirectories, bool overwriteFile, TArray<uint8> bytes, FString& fileName, FString& fullPath, int32& errorCode, FString& errorMessage) {
	...
	fullPath = FPaths::ConvertRelativePathToFull(dir);

	FString dirOnly = FPaths::GetPath(dir);
	if (!FPaths::DirectoryExists(dirOnly)) {
